...
RectangleShape create_mbb_shape(HilbertNode* current,int offset,int depth){
...
void create_tree_gui(vector<RectangleShape>&rectangles,vector<CircleShape>&points, vector<ConvexShape>&polygons,HilbertNode* current,int offset,int depth){
            ...
            if (event.type == sf::Event::Closed)
                ...
                if(Mouse::isButtonPressed(Mouse::Left) || event.type == sf::Event::MouseWheelScrolled){
                    ...
                    if(mode == Mode::insert_points){
                        ...
                        int h_index = hindex(x_p,y_p,14);

                        cout<<x<<" "<<y<<" "<<h_index<<endl;
                        cout<<x_p<<" "<<y_p<<" "<<h_index<<endl;
