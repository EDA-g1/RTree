...
struct Node {
  ...
  bool is_overflown() const { return this->children.size() > M; }
  ...
  void adjustMBB() {
    MBB *mbb = (MBB *)obj;
    int max_y = 0, max_x = 0, min_y = numeric_limits<int>::max(),
  ...
  int choose_split_axis() {
    ...
    // Lambda functions
    auto sort_low_x = [](Node *a, Node *b) {
      return a->obj->getLowX() <= b->obj->getLowX();
    };
    auto sort_low_y = [](Node *a, Node *b) {
      return a->obj->getLowY() <= b->obj->getLowY();
    };
    auto sort_high_x = [](Node *a, Node *b) {
      return a->obj->getHighX() <= b->obj->getHighX();
    };
    auto sort_high_y = [](Node *a, Node *b) {
  ...
  void split_min_index_cost(int &return_index, double &return_overlap,
                            double &return_area) {
  ...
  void set_as_children(Node *u) {
    ...
    if (obj == nullptr) {
      // Create if none
      obj =
          (SpatialObj *)new MBB(Point(u->obj->getLowX(), u->obj->getLowY()),
...
Node *insert(Node *&u, Node *node, int level, int depth,
             vector<bool> &reinsertion_levels);
...
bool contains(SpatialObj *obj, SpatialObj *other) {
  ...
  return ((obj->getLowX() <= other->getLowX() &&
           other->getLowX() <= obj->getHighX()) &&
          (obj->getLowX() <= other->getHighX() &&
           other->getHighX() <= obj->getHighX()) &&
          (obj->getLowY() <= other->getLowY() &&
           other->getLowY() <= obj->getHighY()) &&
          (obj->getLowY() <= other->getHighY() &&
           other->getHighY() <= obj->getHighY()));
}
...
void reinsert(Node *&u, int level, vector<bool> &reinsertion_levels) {
  ...
  auto sort_center_dist = [center](Node *a, Node *b) {
    return a->obj->getDistanceTo((SpatialObj *)&center) >=
           b->obj->getDistanceTo((SpatialObj *)&center);
  };
  ...
  vector<Node *> reinsert_children(u->children.begin(),
                                   u->children.begin() + split);
...
void show(Node *node, string prefix) {
  ...
  if (node->status == Status::leaf_mbb) {
    for (auto &child : node->children) {
      ...
      //cout << endl;
      // << "POINT: ";
      // cout << " (" << child->obj->getLowX() << "," << child->obj->getLowY()
      // << ") " << endl;
    }
  } else {
    for (auto &child : node->children) {
...
public:
  ...
  vector<Node *> knn(SpatialObj *source, int n) {
    ...
    while (!pq.empty()) {
      ...
      if (front.second->status == Status::polygon ||
          front.second->status == Status::point) {
  ...
  void remove_spatialobj(SpatialObj *click, SpatialObj *click_box) {
    if (root->obj == nullptr ||
        (root->status == Status::leaf_mbb && root->children.size() == 0))
      return;
    ...
    if ((supposed_node_to_delete->status == Status::polygon &&
         supposed_node_to_delete->obj->intersection(click_box) > 0) ||
        (supposed_node_to_delete->status == Status::point &&
         contains(click_box, supposed_node_to_delete->obj))) {

      eraseObject(supposed_node_to_delete->parent->children,
                  supposed_node_to_delete->obj);
